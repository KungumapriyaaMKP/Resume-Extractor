...
const PixelTrail: React.FC<PixelTrailProps> = ({
  ...
  React.useEffect(() => {
    const handleGlobalMouseMove = (e: MouseEvent) => {
      ...
      // Check if mouse is over an element that shouldn't show the trail
      ...
      const x = Math.min(
        columns - 1,
        Math.max(0, Math.floor((e.clientX - rect.left) / pixelSize))
      )
      const y = Math.min(
        rows - 1,
        Math.max(0, Math.floor((e.clientY - rect.top) / pixelSize))
      )

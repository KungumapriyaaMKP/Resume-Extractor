...
export default function TetrisLoading({ 
  ...
  const sizeConfig = {
    ...
    lg: { cellSize: 'w-4 h-4', gridWidth: 10, gridHeight: 16, padding: 'p-1.5' }
  }
        ...
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-white/5 transition-all duration-100 ${
              cell.filled 
                ? `${cell.color} scale-100 shadow-[0_0_8px_rgba(255,255,255,0.4)]` 
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="mb-4">
        <div className={`border border-white/10 bg-black/40 backdrop-blur-sm ${config.padding} transition-colors rounded-sm`}>
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-white/80 text-sm font-medium tracking-wide transition-colors">{loadingText}</p>
        </div>
      )}
    </div>
  )
